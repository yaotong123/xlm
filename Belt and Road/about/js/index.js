...
if(USER_AGENT.match(/MSIE 8./) || USER_AGENT.match(/MSIE 7./) || USER_AGENT.match(/MSIE 6./) || USER_AGENT.match(/MSIE 9./)) {
	var weReadIndexIE8 = function() {
		var e = {
			...
			setStyle: function() {
				$('#load').remove();
				$('.pageControl')[0].style.filter = 'progid:DXImageTransform.Microsoft.AlphaImageLoader(src="images/ie/ie_pageControl.png")alpha(opacity=100)';
				$('.pageControl')[1].style.filter = 'progid:DXImageTransform.Microsoft.AlphaImageLoader(src="images/ie/ie_pageControl.png")alpha(opacity=40)';
				$('.pageControl')[2].style.filter = 'progid:DXImageTransform.Microsoft.AlphaImageLoader(src="images/ie/ie_pageControl.png")alpha(opacity=40)';
				$('.pageControl')[3].style.filter = 'progid:DXImageTransform.Microsoft.AlphaImageLoader(src="images/ie/ie_pageControl.png")alpha(opacity=40)';
				$('.pageControl')[4].style.filter = 'progid:DXImageTransform.Microsoft.AlphaImageLoader(src="images/ie/ie_pageControl.png")alpha(opacity=40)';
				$('#background').remove();
				$('#view').prepend('<img id="background_ie" src="images/background.jpg">');
				$('#page_download_hand').remove();
				$('#page_download').prepend('<img id="page_download_hand_ie" src="images/download_hand.png">');
				$('#logo').remove();
				$('#view').prepend('<img id="logo_ie" src="images/logo.png">');
				$('#arrow_right').remove();
				$('#view').prepend('<img id="arrow_right" src="images/ie/ie_arrow_right.png">');
				$('#arrow_left').remove();
				$('#view').prepend('<img id="arrow_left" src="images/ie/ie_arrow_left.png">');
				$('#page_function1_show').remove();
				$('#page_function1').prepend('<img id="page_function1_show" src="images/ie/ie_function1.png">');
				$('#page_function2_show').remove();
				$('#page_function2').prepend('<img id="page_function2_show" src="images/ie/ie_function2.png">');
				$('#page_function3_show').remove();
				$('#page_function3').prepend('<img id="page_function3_show" src="images/ie/ie_function3.png">');
				$('#page_function4_table').remove();
				$('#page_function4').prepend('<img id="page_function4_table" src="images/ie/ie_function4.png">');
				if(USER_AGENT.match(/MSIE 9./)) {} else {
					$('#logo').remove();
					$('#view').prepend('<img id="logo_ie" src="images/logo.png">');
				}
			},
			regEvent: function() {
				...
				$('#nav_QRCode').on('mouseenter', function() {
					...
					$('#nav_QRCode_hover').css({
						'display': 'block'
					}).animate({
						'top': '40px'
					});
					$('#nav_QRCode_hover_border').css({
						'display': 'block'
					}).animate({
						'opacity': '1'
					});
					$('#nav_QRCode_hover_img').animate({
						'opacity': '1'
					});
		...
		function d() {
			...
			if(c === 4) {
				...
				$('#background_ie').animate({
					'left': '-' + ((8.25 * document.documentElement.clientHeight) - document.documentElement.clientWidth - 50) + 'px'
				});
			} else {
				$('#arrow_right').css({
					'display': 'block'
			...
			setTimeout(function() {
				$('.pageControl').css({
					'filter': 'progid:DXImageTransform.Microsoft.AlphaImageLoader(src="images/ie/ie_pageControl.png")alpha(opacity=40)'
				});
				$('.pageControl')[c].style.filter = 'progid:DXImageTransform.Microsoft.AlphaImageLoader(src="images/ie/ie_pageControl.png")alpha(opacity=100)';
			}, 400);
			$('#page').animate({
				'left': '-' + 100 * c + '%'
			});
	...
	var weRead = function(g) {
		var ay = {
			...
			setGlobal: function() {
				var aC = window.innerHeight,
					aD = window.innerWidth;
			...
			checkPageSize: function() {
				var aF = window.innerHeight,
					aG = window.innerWidth,
					aE = ~~(aG / aF * 100);
			...
			switchStyle: function(aC) {
				switch(aC) {
					case 'pc-vertical':
						...
						break;
					case 'pc-horizontal':
						M.style.backgroundImage = 'url("images/function4_table_background_4x.png")';
						aA([al, ak, aj, d, e]);
						if(ao !== 0) {
		...
		var af = g.getElementById('logo'),
			ap = g.getElementById('page'),
			au = g.getElementsByClassName('page'),
			aB = g.getElementById('view'),
			f = g.getElementById('background'),
			h = g.getElementById('page_download_hand'),
			i = g.getElementById('page_download_text'),
			p = g.getElementById('page_function1_text_title'),
			o = g.getElementById('page_function1_text_sub'),
			C = g.getElementById('page_function2_text_title'),
			B = g.getElementById('page_function2_text_sub'),
			L = g.getElementById('page_function3_text_title'),
			K = g.getElementById('page_function3_text_sub'),
			Y = g.getElementById('page_function4_text_title'),
			X = g.getElementById('page_function4_text_sub'),
			aq = g.getElementById('pageControl'),
			ar = g.getElementsByClassName('pageControl');
		var ax = g.getElementById('page_download_pc_button'),
			ah = g.getElementById('page_download_button'),
			ai = g.getElementById('page_download_mobile_iPhone'),
			ag = g.getElementById('page_download_mobile_android');
		var j = g.getElementById('page_function1_book_page1'),
			k = g.getElementById('page_function1_book_page2'),
			l = g.getElementById('page_function1_book_page3'),
			n = g.getElementById('page_function1_skin'),
			m = g.getElementById('page_function1_font');
		var x = g.getElementById('page_function2_face_left'),
			z = g.getElementById('page_function2_face_right'),
			y = g.getElementById('page_function2_face_main'),
			A = g.getElementById('page_function2_face_text'),
			q = g.getElementById('page_function2_book_01'),
			r = g.getElementById('page_function2_book_02'),
			s = g.getElementById('page_function2_book_03'),
			t = g.getElementById('page_function2_book_04'),
			u = g.getElementById('page_function2_book_05'),
			v = g.getElementById('page_function2_book_06'),
			w = g.getElementById('page_function2_book_07');
		var D = g.getElementById('page_function3_book'),
			E = g.getElementById('page_function3_talk_01'),
			F = g.getElementById('page_function3_talk_02'),
			G = g.getElementById('page_function3_talk_03'),
			H = g.getElementById('page_function3_talk_04'),
			I = g.getElementById('page_function3_talk_05'),
			J = g.getElementById('page_function3_talk_06');
		var M = g.getElementById('page_function4_table_background'),
			N = g.getElementById('page_function4_table_column_01'),
			O = g.getElementById('page_function4_table_column_02'),
		...
		var aj = g.getElementById('nav_android'),
			ak = g.getElementById('nav_iPhone'),
			al = g.getElementById('nav_QRCode'),
			am = g.getElementById('nav_QRCode_hover'),
			d = g.getElementById('arrow_left'),
			e = g.getElementById('arrow_right'),
			c = g.getElementById('arrow_down');
		var ao = 0;
		...
		function at() {
			...
			if(!GLOBAL.HORIZONTAL_STYLE) {
				...
				aA(f);
				if(ao === 4) {
					var aL = window.innerHeight,
						aM = window.innerWidth,
						aK = ~~(aM / aL * 100);
...
var shareTimeLineConfig = {
	img_url: "images/weread_icon.png",
	link: window.location.href,
	desc: "",
	title: "\u5FAE\u4FE1\u8BFB\u4E66\uFF0C\u8BA9\u9605\u8BFB\u4E0D\u518D\u5B64\u72EC\u3002"
};
var shareMessageConfig = {
	img_url: "images/weread_icon.png",
	link: window.location.href,
	desc: "\u8BA9\u9605\u8BFB\u4E0D\u518D\u5B64\u72EC",
	title: "\u5FAE\u4FE1\u8BFB\u4E66"
